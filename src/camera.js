...
export function createCamera(gameWindow) {
  ...
  function onMouseMove(event) {
    ...
    if (isLeftMouseDown && !event.shiftKey && !event.altKey) {
    ...
    if (isMiddleMouseDown || event.altKey && isLeftMouseDown) {
      const forward = new THREE.Vector3(0, 0, 1).applyAxisAngle(Y_AXIS, cameraAzimuth * DEG2RAD);
      const left = new THREE.Vector3(1, 0, ).applyAxisAngle(Y_AXIS, cameraAzimuth * DEG2RAD);
    ...
    if (isRightMouseDown || event.shiftKey && isLeftMouseDown) {
